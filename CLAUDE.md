...
dotnet test  source/OpenTPW.sln --no-build          # ~1 s. Without the game: 431 pass, 512 skip
OPENTPW_GAME_PATH="<game folder>" dotnet test source/OpenTPW.sln --no-build   # all 943
...
- Tests that need the game call `GameData.Required()` and skip when it is absent. A green run with 512 skips is normal without the game.
