...
17. **Show the next five queue items before every clear** (2026-09-23; replaces showing `docs/PLAYER-GAPS.md`). When Alexah says they are about to clear the session, show the next five unticked items in `docs/QUEUE.md`, in queue order, each with what it asks for. **Read the file and show what it says**, rather than reciting it from memory; a half-remembered list handed over at a clear is the one place a stale claim cannot be caught later.

## Build, test, run

```sh
dotnet build source/OpenTPW.sln --no-incremental    # 0 errors expected; ~125 warnings are known, leave them
dotnet test  source/OpenTPW.sln --no-build          # ~1 s. Without the game: 431 pass, 515 skip
OPENTPW_GAME_PATH="<game folder>" dotnet test source/OpenTPW.sln --no-build   # all 946
...
- Tests that need the game call `GameData.Required()` and skip when it is absent. A green run with 515 skips is normal without the game.
