...
**And rides that run their own scripts.** Every placed thing runs its own compiled script - 63 of the 106 instructions the opcode table declares, with the rest counted rather than guessed at - and a ride takes its turn from the same sweep the people take theirs from, inviting whoever is at the front of its queue. Only two of Lost Kingdom's things can be offered to a guest at all - the sideshow and the ride - because the other four carrying the "a guest may choose this" bit declare no queue cells, and an object with no queue cells has no room by the game's own arithmetic.

**Nobody actually rides yet, though.** The boarding chain is wired and its arithmetic is covered by tests, but six of the twenty-six states a guest can be in have no handler, and two of those are the riding states themselves - so a guest who boards stops there rather than going round and getting off. This paragraph claimed they rode; that was measured in a test harness and is not what happens on screen.
