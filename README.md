...
**It is not playable yet.** You can explore the lobby and the front end - pick a park, look around the islands, hear the advisor - and you can now enter a park and look around it, ground and scenery and the shops and rides it was laid out with. A park has begun to *run*: its clock ticks, its weather turns, the guests and staff its save was holding walk about in it, and its one ride carries people. But most of what a park does is still missing - nothing is bought, built or hired, and no shop serves anyone. It has the management gadget the original puts in its corner, with a live date on it, but four of that gadget's six buttons have no screen to open yet. See [Status](#status) for the full picture.
...
**And the people it was saved with.** The thirteen guests and five staff the save was holding are read out of it, drawn as the sprites they wear, and take their turns on the engine's own schedule. A guest walks the route the save gave them, forms an opinion of the admission fee, pays it, picks somewhere to go, walks there across the real map, joins the queue and steps up it as it moves. The staff run one shared machine with five extensions: the guard and the researcher patrol beats of their own, and a worn-out one of any kind finds the park's rest area and walks to the cell it is approached from.

**And a ride that carries them.** Every placed thing runs its own compiled script - 63 of the 106 instructions the opcode table declares, with the rest counted rather than guessed at - and a ride takes its turn from the same sweep the people take theirs from. It invites whoever is at the front of its queue, admits them, carries them for the duration its own file asks for, lets them off at its exit rather than its entrance, and charges them on the way out. That is one ride, in one park: the Belly Bounce in Lost Kingdom. Only two of that park's things can be offered to a guest at all - the sideshow and the ride - because the other four that carry the "a guest may choose this" bit declare no queue cells, and an object with no queue cells has no room by the game's own arithmetic.

**What does not.** Most of a park still does not work. A shop serves nobody - it declares no queue cells, so nothing can ever be sent to it - and nothing in the game buys an attraction, hires anyone, or shows the finances. The handyman, the mechanic and the entertainer finish the walk the save left them on and stand, because their work wants litter, breakdowns and an audience: this park holds no litter at all in any of its 16,384 cells, so a working handyman would correctly find nothing. The gadget draws, its date runs and its happiness gauge moves, and the camera and map buttons open what they promise - but the message bar is deliberately absent, because nothing in this game can put a message in it, and four of the six buttons have no screen behind them. Its sound is its music, its rain and its thunder and nothing else: the ambience, the ride sounds and the park speech are all things the original starts from places this does not read yet, the ambience in particular being a list of emitters placed around the world in the level's own `scape.omp`. Nothing writes a park back - no saved park has ever been read by anything but this, so the reader is not assumed to generalise. There is no video, and nothing online: there is no networking code in the project at all.
