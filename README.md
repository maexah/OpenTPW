...
**What does not.** Leaving a park once you are in one - only the debug console can. The objects a park places now stand in it, but none of them *works*: a ride is scenery, a shop serves nobody, and there are no staff and no visitors - the queue leading to a ride is drawn, but nobody stands in it. Nor is there a park interface, ride scripts, video, or anything online - there is no networking code in the project at all.
