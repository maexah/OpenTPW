...
**What does not.** The objects a park places now stand in it, but none of them *works*: a ride is scenery, a shop serves nobody, and there are no staff and no visitors - the queue leading to a ride is drawn, but nobody stands in it. Beyond that menu a park has no interface at all - no HUD, no advisor, no weather and none of its own sound - and there are no ride scripts, no video, and nothing online: there is no networking code in the project at all.
...
Forty-six of the ninety unit tests read real game files and skip when no installation is found - so a green run on a machine that has never had the game means 44 ran and 46 did not. Set `OPENTPW_GAME_PATH` to run all of them.
