...
Four hundred and eighty-seven of the nine hundred and fifteen unit tests read real game files and skip when no installation is found - so a green run on a machine that has never had the game means **428 ran and 487 did not**. Set `OPENTPW_GAME_PATH` to run all of them. (Take these fresh; they move most sessions.)
